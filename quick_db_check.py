...
db_path = (
    r"c:\Users\Brenda\Desktop\BrendaDevs\Mykonos-os\src\backend\database\mykonos.db"
)
...
if os.path.exists(db_path):
    ...
    if "warehouse_stock_variants" in tables:
        ...
        if variants_count > 0:
            ...
            for sample in samples:
                print(
                    f"  ID: {sample[0]}, Producto: {sample[1]}, Barcode: '{sample[2]}', Status: {sample[3]}"
                )
        else:
    ...
    if "warehouse_stock" in tables:
        ...
        if stock_count > 0:
            cursor.execute(
                "SELECT product_id, branch_id, quantity FROM warehouse_stock WHERE quantity > 0 LIMIT 3"
            )
            stock_samples = cursor.fetchall()
            print("Muestras de stock tradicional:")
            for sample in stock_samples:
                print(
                    f"  Producto: {sample[0]}, Sucursal: {sample[1]}, Cantidad: {sample[2]}"
                )
