...
export default function RegistroVentas() {
  ...
  const searchGiftByBarcode = async (barcode) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setGiftSearchResult(data.gift_info)
        toast.success(data.message)

        // También cargar los detalles completos de la venta
        if (data.gift_info.sale_info.sale_id) {
          await loadSaleDetails(data.gift_info.sale_info.sale_id)
        }
      } else {
        setGiftSearchResult(null)
        toast.error(data.error || 'No se encontró información para este código de regalo')
      }
    } catch (error) {
      console.error('Error buscando regalo:', error)
      setGiftSearchResult(null)
      toast.error('Error al buscar información del regalo')
    } finally {
      setSearchingGift(false)
    }
  }
            ...
            <div className="flex flex-1 flex-col gap-4">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {searchingGift && isGiftSearch && (
                    <span className="loading loading-spinner loading-sm"></span>
                  )}
                </label>

                {/* Clear button */}
                {(searchTerm || giftBarcode) && (
                  <button
                    type="button"
