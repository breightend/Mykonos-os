...
with open("test_results.txt", "w") as f:
    try:
        ...
        # Get table schema
        cursor.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='proveedorxmarca'"
        )
        ...
        if entity_result.get("success"):
            ...
            if brand_result.get("success"):
                ...
                relationship_data = {"id_brand": brand_id, "id_provider": provider_id}

                rel_result = db.add_record(
                    TABLES.PROVEEDORXMARCA.value, relationship_data
                )
                f.write(f"Relationship add result: {rel_result}\n")

                if rel_result.get("success"):
                    f.write(
                        "✓ Composite primary key relationship created successfully!\n"
                    )

                    # Try to add duplicate (should fail)
                    duplicate_result = db.add_record(
                        TABLES.PROVEEDORXMARCA.value, relationship_data
                    )
                    ...
                    exists = db.check_provider_brand_relationship_exists(
