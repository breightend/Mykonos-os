...
def check_database():
    ...
    print("\n=== WAREHOUSE_STOCK ===")
    cursor.execute(
        "SELECT product_id, branch_id, quantity FROM warehouse_stock ORDER BY product_id DESC LIMIT 10"
    )
    ...
    cursor.execute(
        "SELECT product_id, size_id, color_id, quantity FROM warehouse_stock_variants ORDER BY product_id DESC LIMIT 10"
    )
    ...
    if products:
        ...
        cursor.execute(
            "SELECT * FROM warehouse_stock WHERE product_id = ?", (last_product_id,)
        )
