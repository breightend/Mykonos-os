...
def populate_test_data():
    ...
    try:
            ...
            for size in sizes:
                for color in colors:
                    ...
                    if result.get("success"):
                        print(
                            f"  ✅ {size['size_name']} + {color['color_name']}: 5 unidades"
                        )
                    else:
        ...
        print(
            f"   Registros encontrados: {len(variants_result) if variants_result else 0}"
        )

        if variants_result:
            for v in variants_result:
                print(
                    f"   - {v.get('size_name')} + {v.get('color_name')}: {v.get('quantity')} unidades"
                )
