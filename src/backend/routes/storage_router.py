...
def create_storage():
    ...
    if success:
        ...
        return jsonify({"mensaje": "Error al crear la sucursal", "status": "error"}), 500
...
def update_storage(storage_id):
    ...
    success = db.update_record("storage", {
        "id": storage_id,
        "name": name,
        "address": address,
        "postal_code": postal_code,
    ...
    if success:
        return jsonify({"mensaje": "Sucursal actualizada con éxito", "status": "éxito"}), 200
    else:
        return jsonify({"mensaje": "Error al actualizar la sucursal", "status": "error"}), 500
...
def delete_storage(storage_id):
    ...
    if success:
        return jsonify({"mensaje": "Sucursal eliminada con éxito", "status": "éxito"}), 200
    else:
        return jsonify({"mensaje": "Error al eliminar la sucursal", "status": "error"}), 500
...
def get_storage_employees(storage_id):
    ...
    try:
        ...
        return jsonify({"mensaje": "Error al obtener empleados de la sucursal", "status": "error"}), 500
...
def assign_employee_to_storage(storage_id):
    ...
    success = db.add_record("usersxstorage", {
        "id_user": user_id,
        "id_storage": storage_id
    })
    
    if success:
        return jsonify({"mensaje": "Empleado asignado a sucursal con éxito", "status": "éxito"}), 200
    else:
        return jsonify({"mensaje": "Error al asignar empleado a sucursal", "status": "error"}), 500
...
def remove_employee_from_storage(storage_id, user_id):
    db = Database()
    success = db.delete_record("usersxstorage", "id_user = ? AND id_storage = ?", user_id, storage_id)
    if success:
        return jsonify({"mensaje": "Empleado removido de sucursal con éxito", "status": "éxito"}), 200
    else:
        return jsonify({"mensaje": "Error al remover empleado de sucursal", "status": "error"}), 500
