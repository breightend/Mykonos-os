...
def test_variants():
    ...
    try:
        ...
        count_result = db.execute_query("SELECT COUNT(*) as count FROM warehouse_stock_variants")
        ...
        if count > 0:
            ...
            for i, record in enumerate(sample):
                print(f"  {i+1}. Product: {record.get('product_id')}, Size: {record.get('size_id')}, Color: {record.get('color_id')}, Quantity: {record.get('quantity')}")
        ...
        if count == 0 and products and sizes and colors and storage:
            ...
            if result.get("success"):
                print(f"✅ Creado registro de ejemplo: Producto {product_id}, Talle {size_id}, Color {color_id}, Cantidad 10")
            else:
        ...
        if products:
            ...
            print(f"📊 Query resultado para producto {product_id}: {len(variants_result) if variants_result else 0} registros")
            
            if variants_result:
                print("📋 Datos encontrados:")
                for v in variants_result[:3]:
                    print(f"  - Talle: {v.get('size_name')}, Color: {v.get('color_name')}, Cantidad: {v.get('quantity')}")
            else:
