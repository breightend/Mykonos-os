...
def debug_backend_data():
    ...
    try:
        ...
        cursor.execute(
            "SELECT DISTINCT product_id FROM warehouse_stock_variants LIMIT 1"
        )
        ...
        cursor.execute(
            "SELECT id, variant_barcode FROM warehouse_stock_variants WHERE product_id = ? LIMIT 5",
            (product_id,),
        )
        ...
        for record in direct_check:
            ...
            print(
                f"   ID {record[0]}: variant_barcode = '{barcode_value}' (tipo: {type(barcode_value)})"
            )
