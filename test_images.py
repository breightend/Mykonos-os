...
for product_id in range(1, 21):  # Test first 20 products
    try:
        ...
        if response.status_code == 200:
            print(
                f"✅ Producto {product_id} SÍ tiene imagen (Content-Type: {response.headers.get('content-type')})"
            )
...
try:
    ...
    if inventory_response.status_code == 200:
        ...
        products_with_images = [
            p for p in products.get("data", []) if p.get("has_image", False)
        ]
        print(f"📋 Productos que reportan tener imagen: {len(products_with_images)}")
        for p in products_with_images[:5]:  # Show first 5
            print(
                f"   - ID: {p.get('id')}, Nombre: {p.get('product_name')}, has_image: {p.get('has_image')}"
            )
except Exception as e:
