...
def deep_debug_product_14():
    ...
    try:
        ...
        if direct_result and len(direct_result) > 0:
            ...
            print(f"   ✅ Encontrado - Product ID: {record['product_id'] if isinstance(record, dict) else record[0]}")
            print(f"   ✅ Tamaño: {record['size'] if isinstance(record, dict) else record[1]} bytes")
