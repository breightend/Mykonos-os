...
def check_product_14():
    ...
    try:
        ...
        product_query = (
            "SELECT id, product_name, images_ids FROM products WHERE id = 14"
        )
        ...
        if product_result:
            ...
            print(
                f"   ID: {product['id'] if isinstance(product, dict) else product[0]}"
            )
            print(
                f"   Nombre: {product['product_name'] if isinstance(product, dict) else product[1]}"
            )
            print(
                f"   images_ids: {product['images_ids'] if isinstance(product, dict) else product[2]}"
            )
        else:
        ...
        if image_result and len(image_result) > 0:
            ...
            print(
                f"   product_id: {image['product_id'] if isinstance(image, dict) else image[0]}"
            )
            print(
                f"   Tamaño imagen: {image['size'] if isinstance(image, dict) else image[1]} bytes"
            )
