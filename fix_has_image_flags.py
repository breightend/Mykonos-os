...
def fix_has_image_flags():
    ...
    try:
        ...
        for product in products:
            ...
            product_name = (
                product["product_name"] if isinstance(product, dict) else product[1]
            )
            current_images_ids = (
                product["images_ids"] if isinstance(product, dict) else product[2]
            )

            # Verificar si realmente tiene imagen en la tabla images
            image_check_query = (
                "SELECT COUNT(*) as count FROM images WHERE product_id = %s"
            )
            ...
            if image_result and len(image_result) > 0:
                count = (
                    image_result[0]["count"]
                    if isinstance(image_result[0], dict)
                    else image_result[0][0]
                )
            ...
            if (current_images_ids is None and actual_has_image) or (
                current_images_ids is not None and not actual_has_image
            ):
                print(
                    f"🔧 Corrigiendo producto {product_id} ({product_name}): has_image -> {actual_has_image}"
                )
                ...
                print(
                    f"✅ Producto {product_id} ({product_name}): correcto (has_image = {actual_has_image})"
                )
...
def verify_corrections():
    ...
    try:
        ...
        print(
            f"❌ Falsos positivos (dicen tener imagen pero no la tienen): {len(false_positives) if false_positives else 0}"
        )
        if false_positives:
        ...
        print(
            f"❌ Falsos negativos (tienen imagen pero no están marcados): {len(false_negatives) if false_negatives else 0}"
        )
        if false_negatives:
        ...
        if (not false_positives or len(false_positives) == 0) and (
            not false_negatives or len(false_negatives) == 0
        ):
