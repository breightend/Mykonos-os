...
def ensure_default_sizes_colors(cursor, conn):
    ...
    try:
        ...
        if sizes_count == 0:
            ...
            cursor.executemany("INSERT INTO sizes (size_name) VALUES (?)", default_sizes)
        ...
        if colors_count == 0:
            ...
            cursor.executemany("INSERT INTO colors (color_name, color_hex) VALUES (?, ?)", default_colors)
...
def migrate_warehouse_data(cursor, conn):
    ...
    try:
        ...
        for product_id, branch_id, quantity in stock_records:
            ...
            variant_barcode = f"VAR-{product_id}-{default_size_id}-{default_color_id}-{branch_id}"
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (product_id, branch_id, default_size_id, default_color_id, quantity, variant_barcode, datetime.now().isoformat()))
...
def fix_missing_variant_barcodes(cursor, conn):
    ...
    try:
        ...
        if missing_barcodes:
            print(f"🔧 Corrigiendo {len(missing_barcodes)} códigos de barras faltantes...")
            
            for variant_id, product_id, size_id, color_id, branch_id in missing_barcodes:
...
def show_sample_variants(cursor):
    ...
    try:
        ...
        if samples:
            ...
            for row in samples:
                variant_id, product_id, product_name, size_name, color_name, sucursal, quantity, variant_barcode = row
                print(f"   ID: {variant_id} | Producto: {product_name or 'Sin nombre'} ({product_id})")
                print(f"      Talle: {size_name or 'Sin talle'} | Color: {color_name or 'Sin color'}")
                print(f"      Sucursal: {sucursal or 'Sin sucursal'} | Cantidad: {quantity}")
